...
public class IolistService {
	...
	public void viewIolist() {
		...
		System.out.println("================================================================================");
		System.out.println("우리동네 경영마트 - 매입매출 리스트");
		System.out.println("--------------------------------------------------------------------------------");
	...
	public boolean insertIO() {
		...
		IolistVO vo = new IolistVO(0, strDate, strPcode, 
							strCcode, strInout, intQty, 
							intPrice, intTotal);
	...
	public boolean UpdateIO() {
		...
		/*
		 * 일단 거래처 코드를 보여주고
		 * 거래처 코드를 변경하려면 코드를 입력 후 Enter
		 * 변경하지 않으려면 그냥 Enter
		 * 
		 *  코드를 입력하지 않고 Enter를 누르면
		 *  strCcode는 아무 값도 없다.
		 *  이때 strCcode.isEmpty() 는 true를 return한다.
		 *  
		 *  따라서 코드를 입력 후 Enter를 누루면
		 *  strCcode.isEmpty()를 skip하여 입력한 코드가 strCcode에 담겨있고
		 *  아무것도 입력하지 않고 Enter를 누르면
		 *  strCcode.isEmpty()가 true가 되어 vo에 담겨있는 io_ccode값을 변순에
		 *  대입하여 원래 거래처코드를 유지하게 된다.
		 */
		...
		String strPrise = scan.nextLine();
		int intPrice = 0;
		if(strPrise.isEmpty()) intPrice = vo.getIo_price();
		else intPrice  = Integer.valueOf(strPrise);
		System.out.printf("수량  ( %d ) >>", vo.getIo_qty());
		String strQty = scan.nextLine();
		
		int intQty = 0;
		if(strQty.isEmpty()) intQty = vo.getIo_qty();
		else intQty = Integer.valueOf(intQty);
